...
class Completed extends Main {
    ...
    public function __construct() {
        add_filter('sejoli/notification/fields',    [$this, 'add_setting_fields'], 60);
    }
    ...
    public function add_setting_fields(array $fields) {
        ...
        $fields['completed'] = [
			'title'  => __('Order selesai', 'sejoli'),
			'fields' => [

                // Untuk buyer
				Field::make( 'html', 'sejoli_completed_shortcode_html', __( 'Shortcode' ) )
                ...
                Field::make('separator'	,'sep_completed_email', 	__('Email' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media email', 'sejoli')),
				Field::make('text', 	'completed_email_title',	 __('Judul' ,'sejoli'))
					->set_required(true)
					->set_default_value(__('{{buyer-name}}, Order #{{invoice-id}} {{product-name}} Anda telah selesai', 'sejoli')),
				Field::make('rich_text', 'completed_email_content', __('Konten', 'sejoli'))
					->set_required(true)
					->set_default_value(sejoli_get_notification_content('order-completed-customer')),

				Field::make('separator'	,'sep_completed_sms', 	__('SMS' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media sms', 'sejoli')),
				Field::make('textarea', 'completed_sms_content', __('Konten', 'sejoli')),

				Field::make('separator'	,'sep_completed_whatsapp', 	__('WhatsApp' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media whatsapp', 'sejoli')),
				Field::make('textarea', 'completed_whatsapp_content', __('Konten', 'sejoli')),

                // Untuk admin
				Field::make('separator', 	'sep_completed_admin', 		__('Konten untuk admin dan lainnya','sejoli'))
					->set_classes('main-title'),

				Field::make('checkbox',		'completed_admin_active',	__('Aktifkan notifikasi untuk admin', 'sejoli'))
                    ->set_default_value(true),

				Field::make('separator',	'sep_completed_admin_email', 	__('Email' ,'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'completed_admin_active',
							'value' => true,
						]
					]),

				Field::make('text',		'completed_admin_email_recipient', __('Alamat Email Penerima', 'sejoli'))
				...
				Field::make('text', 		'completed_admin_email_title',	 __('Judul' ,'sejoli'))
					->set_required(true)
					->set_default_value(__('Order #{{invoice-id}} {{product-name}} dari {{buyer-name}} telah selesai', 'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'completed_admin_active',
							'value' => true,
				...
				Field::make('rich_text', 'completed_admin_email_content', __('Konten', 'sejoli'))
					->set_required(true)
					->set_default_value(sejoli_get_notification_content('order-completed-admin'))
					->set_conditional_logic([
						[
							'field'	=> 'completed_admin_active',
							'value' => true,
				...
				Field::make('separator'	,'sep_completed_admin_sms', 	__('SMS' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media sms', 'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'completed_admin_active',
				...
				Field::make('text',		'completed_admin_sms_recipient', __('Nomor SMS Penerima', 'sejoli'))
					->set_help_text(__('Gunakan tanda koma jika penerima ada lebih dari 1', 'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'completed_admin_active',
				...
				Field::make('textarea', 'completed_admin_sms_content', __('Konten', 'sejoli'))
					->set_help_text(__('Dengan mengosongkan isian ini, tidak akan ada notifikasi yang dikirimkan via sms', 'sejoli'))
					->set_default_value(sejoli_get_notification_content('order-completed-admin', 'sms'))
					->set_conditional_logic([
						[
							'field'	=> 'completed_admin_active',
							'value' => true,
				...
				Field::make('separator'	,'sep_completed_admin_whatsapp', 	__('WhatsApp' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media whatsapp', 'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'completed_admin_active',
				...
				Field::make('text',		'completed_admin_whatsapp_recipient', __('Nomor WhatsApp Penerima', 'sejoli'))
					->set_help_text(__('Gunakan tanda koma jika penerima ada lebih dari 1', 'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'completed_admin_active',
				...
				Field::make('textarea', 'completed_admin_whatsapp_content', __('Konten', 'sejoli'))
					->set_help_text(__('Dengan mengosongkan isian ini, tidak akan ada notifikasi yang dikirimkan via whatsapp', 'sejoli'))
					->set_default_value(sejoli_get_notification_content('order-completed-admin', 'whatsapp'))
					->set_conditional_logic([
						[
							'field'	=> 'completed_admin_active',
							'value' => true,
    ...
    protected function set_content() {
		...
		$this->set_recipient_title    ('buyer', 'email',  sejolisa_carbon_get_theme_option('completed_email_title'));
		$this->set_recipient_content  ('buyer', 'email',  $this->set_notification_content(
    															carbon_get_theme_option('completed_email_content'),
    															'email',
                                                                'buyer'
    														));

		if(!empty(sejolisa_carbon_get_theme_option('completed_whatsapp_content'))) :

            $this->set_enable_send('whatsapp', 'buyer', true);
			$this->set_recipient_content  ('buyer', 'whatsapp',  $this->set_notification_content(
    				                                                sejolisa_carbon_get_theme_option('completed_whatsapp_content'),
    				                                                'whatsapp',
                                                                    'buyer'
                                                                ));
        endif;

		if(!empty(sejolisa_carbon_get_theme_option('completed_sms_content'))) :
            $this->set_enable_send('sms', 'buyer', true);
			$this->set_recipient_content  ('buyer', 'sms',  $this->set_notification_content(
                                                				sejolisa_carbon_get_theme_option('completed_sms_content'),
                                                				'sms',
                                                                'buyer'
                                                			));
        endif;
		...
		if(false !== sejolisa_carbon_get_theme_option('completed_admin_active')) :

            $this->set_enable_send('email', 'admin', true);
    		$this->set_recipient_title    ('admin', 'email', sejolisa_carbon_get_theme_option('completed_admin_email_title'));
    		$this->set_recipient_content  ('admin', 'email',  $this->set_notification_content(
        															carbon_get_theme_option('completed_admin_email_content'),
        															'email',
                                                                    'admin'
        														));

    		if(!empty(sejolisa_carbon_get_theme_option('completed_admin_whatsapp_content'))) :

				$this->set_enable_send('whatsapp', 'admin', true);
				$this->set_recipient_content('admin', 'whatsapp',  $this->set_notification_content(
                                                    					carbon_get_theme_option('completed_admin_whatsapp_content'),
                                                    					'whatsapp',
                                                                        'admin'
                                                    				));
            endif;

    		if(!empty(sejolisa_carbon_get_theme_option('completed_admin_sms_content'))) :
                $this->set_enable_send('sms', 'admin', true);
				$this->set_recipient_content('admin', 'sms',  $this->set_notification_content(
                                                					sejolisa_carbon_get_theme_option('completed_admin_sms_content'),
                                                					'sms',
                                                                    'admin'
                                                				));
            endif;
    ...
    protected function check_recipients($media = 'email', $role = 'admin') {
        $recipients       = sejolisa_carbon_get_theme_option('completed_' . $role . '_' . $media . '_recipient');
