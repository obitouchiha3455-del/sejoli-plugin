...
class InProgress extends Main {
    ...
    public function add_setting_fields(array $fields) {
        ...
        $fields['in-progress'] = [
			'title'  => __('Pesanan diproses', 'sejoli'),
			'fields' => [

				// untuk customer
				Field::make( 'html', 'sejoli_procced_shortcode_html', __( 'Shortcode' ) )
                ...
                Field::make('separator', 'sep_in_progress_buyer', __('Konten untuk pembeli','sejoli'))
					->set_classes('main-title'),

				Field::make('separator', 'sep_in_progress_email', 	__('Email' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media email', 'sejoli')),

				Field::make('text', 	'in_progress_email_title',	 __('Judul' ,'sejoli'))
					->set_required(true)
					->set_default_value(__('Selamat, pembelian Anda untuk {{product-name}} Berhasil', 'sejoli')),

				Field::make('rich_text', 'in_progress_email_content', __('Konten', 'sejoli'))
					->set_required(true)
					->set_default_value(sejoli_get_notification_content('order-in-progress-customer')),

				Field::make('separator'	,'sep_in_progress_sms', 	__('SMS' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media sms', 'sejoli')),

				Field::make('textarea', 'in_progress_sms_content', __('Konten', 'sejoli'))
					->set_default_value(sejoli_get_notification_content('order-in-progress-customer', 'sms'))
					->set_help_text(__('Dengan mengosongkan isian ini, tidak akan ada notifikasi yang dikirimkan via sms', 'sejoli')),

				Field::make('separator'	,'sep_in_progress_whatsapp', 	__('WhatsApp' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media whatsapp', 'sejoli')),

				Field::make('textarea', 'in_progress_whatsapp_content', __('Konten', 'sejoli'))
					->set_default_value(sejoli_get_notification_content('order-in-progress-customer', 'whatsapp'))
					->set_help_text(__('Dengan mengosongkan isian ini, tidak akan ada notifikasi yang dikirimkan via whatsapp', 'sejoli')),

				// Untuk admin
				Field::make('separator', 	'sep_in_progress_admin', 		__('Konten untuk admin dan lainnya','sejoli'))
					->set_classes('main-title'),
				Field::make('checkbox',		'in_progress_admin_active',			__('Aktifkan notifikasi untuk admin', 'sejoli')),

				Field::make('separator',	'sep_in_progress_admin_email', 	__('Email' ,'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'in_progress_admin_active',
							'value' => true,
						]
					]),

				Field::make('text',		'in_progress_admin_email_recipient', __('Alamat Email Penerima', 'sejoli'))
				...
				Field::make('text', 		'in_progress_admin_email_title',	 __('Judul' ,'sejoli'))
					->set_required(true)
					->set_default_value(__('Order #{{invoice-id}} {{product-name}} dari {{buyer-name}} selesai dibayarkan. Proses segera', 'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'in_progress_admin_active',
							'value' => true,
				...
				Field::make('rich_text', 'in_progress_admin_email_content', __('Konten', 'sejoli'))
					->set_required(true)
				...
				Field::make('separator'	,'sep_in_progress_admin_sms', 	__('SMS' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media sms', 'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'in_progress_admin_active',
				...
				Field::make('text',		'in_progress_admin_sms_recipient', __('Nomor SMS Penerima', 'sejoli'))
					->set_help_text(__('Gunakan tanda koma jika penerima ada lebih dari 1', 'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'in_progress_admin_active',
				...
				Field::make('textarea', 'in_progress_admin_sms_content', __('Konten', 'sejoli'))
					->set_help_text(__('Dengan mengosongkan isian ini, tidak akan ada notifikasi yang dikirimkan via sms', 'sejoli'))
				...
				Field::make('separator'	,'sep_in_progress_admin_whatsapp', 	__('WhatsApp' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media whatsapp', 'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'in_progress_admin_active',
				...
				Field::make('text',		'in_progress_admin_whatsapp_recipient', __('Nomor WhatsApp Penerima', 'sejoli'))
					->set_help_text(__('Gunakan tanda koma jika penerima ada lebih dari 1', 'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'in_progress_admin_active',
				...
				Field::make('textarea', 'in_progress_admin_whatsapp_content', __('Konten', 'sejoli'))
					->set_help_text(__('Dengan mengosongkan isian ini, tidak akan ada notifikasi yang dikirimkan via whatsapp', 'sejoli'))
    ...
    protected function set_content() {
		...
		$this->set_recipient_title    ('buyer', 'email', sejolisa_carbon_get_theme_option('in_progress_email_title'));
		$this->set_recipient_content  ('buyer', 'email', $this->set_notification_content(
															sejolisa_carbon_get_theme_option('in_progress_email_content'),
															'email',
                                                            'buyer'
														 ));

		if(!empty(sejolisa_carbon_get_theme_option('in_progress_whatsapp_content'))) :

            $this->set_enable_send('whatsapp', 'buyer', true);
			$this->set_recipient_content('buyer', 'whatsapp', $this->set_notification_content(
				                                                sejolisa_carbon_get_theme_option('in_progress_whatsapp_content'),
				                                                'whatsapp',
                                                                'buyer'
                                                              ));
        endif;

		if(!empty(sejolisa_carbon_get_theme_option('in_progress_sms_content'))) :
            $this->set_enable_send('sms', 'buyer', true);
			$this->set_recipient_content('buyer', 'sms', $this->set_notification_content(
                                            				sejolisa_carbon_get_theme_option('in_progress_sms_content'),
                                            				'sms',
                                                            'buyer'
                                            			));
        endif;
		...
		if(false !== sejolisa_carbon_get_theme_option('in_progress_admin_active')) :

            $this->set_enable_send('email', 'admin', true);
    		$this->set_recipient_title    ('admin', 'email',  sejolisa_carbon_get_theme_option('in_progress_admin_email_title'));
    		$this->set_recipient_content  ('admin', 'email',  $this->set_notification_content(
    															sejolisa_carbon_get_theme_option('in_progress_admin_email_content'),
    															'email',
                                                                'admin'
    														  ));

    		if(!empty(sejolisa_carbon_get_theme_option('in_progress_admin_whatsapp_content'))) :

				$this->set_enable_send('whatsapp', 'admin', true);
				$this->set_recipient_content('admin', 'whatsapp',  $this->set_notification_content(
                                                    					sejolisa_carbon_get_theme_option('in_progress_admin_whatsapp_content'),
                                                    					'whatsapp',
                                                                        'admin'
                                                    				));
            endif;

    		if(!empty(sejolisa_carbon_get_theme_option('in_progress_admin_sms_content'))) :
                $this->set_enable_send('sms', 'admin', true);
				$this->set_recipient_content('buyer', 'sms',  $this->set_notification_content(
                                                					sejolisa_carbon_get_theme_option('in_progress_admin_sms_content'),
                                                					'sms',
                                                                    'admin'
                                                				));
            endif;
    ...
    protected function check_recipients($media = 'email', $role = 'admin') {
        $recipients       = sejolisa_carbon_get_theme_option('in_progress_' . $role . '_' . $media . '_recipient');
