...
class Refund extends Main {
    ...
    public function __construct() {
        add_filter('sejoli/notification/fields',    [$this, 'add_setting_fields'], 80);
    }
    ...
    public function add_setting_fields(array $fields) {
        ...
        $fields['refund'] = [
			'title'  => __('Refund', 'sejoli'),
			'fields' => [

				// untuk customer
				Field::make( 'html', 'sejoli_refund_shortcode_html', __( 'Shortcode' ) )
                ...
                Field::make('separator', 'sep_refund_buyer', __('Konten untuk pembeli','sejoli'))
					->set_classes('main-title'),

				Field::make('separator', 'sep_refund_email', 	__('Email' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media email', 'sejoli')),

				Field::make('text', 	'refund_email_title',	 __('Judul' ,'sejoli'))
					->set_required(true)
					->set_default_value(__('{{buyer-name}}, Order #{{invoice-id}} {{product-name}} anda telah kami lakukan refund', 'sejoli')),

				Field::make('rich_text', 'refund_email_content', __('Konten', 'sejoli'))
					->set_required(true)
					->set_default_value(sejoli_get_notification_content('order-refund-customer')),

				Field::make('separator'	,'sep_refund_sms', 	__('SMS' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media sms', 'sejoli')),

				Field::make('textarea', 'refund_sms_content', __('Konten', 'sejoli'))
					->set_default_value(sejoli_get_notification_content('order-refund-customer', 'sms'))
					->set_help_text(__('Dengan mengosongkan isian ini, tidak akan ada notifikasi yang dikirimkan via sms', 'sejoli')),

				Field::make('separator'	,'sep_refund_whatsapp', 	__('WhatsApp' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media whatsapp', 'sejoli')),

				Field::make('textarea', 'refund_whatsapp_content', __('Konten', 'sejoli'))
					->set_default_value(sejoli_get_notification_content('order-refund-customer', 'whatsapp'))
					->set_help_text(__('Dengan mengosongkan isian ini, tidak akan ada notifikasi yang dikirimkan via whatsapp', 'sejoli')),

				// Untuk admin
				Field::make('separator', 	'sep_refund_admin', 		__('Konten untuk admin dan lainnya','sejoli'))
					->set_classes('main-title'),
				Field::make('checkbox',		'refund_admin_active',			__('Aktifkan notifikasi untuk admin', 'sejoli')),

				Field::make('separator',	'sep_refund_admin_email', 	__('Email' ,'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'refund_admin_active',
							'value' => true,
						]
					]),

				Field::make('text',		'refund_admin_email_recipient', __('Alamat Email Penerima', 'sejoli'))
				...
				Field::make('text', 		'refund_admin_email_title',	 __('Judul' ,'sejoli'))
					->set_required(true)
				...
				Field::make('rich_text', 'refund_admin_email_content', __('Konten', 'sejoli'))
					->set_required(true)
				...
				Field::make('separator'	,'sep_refund_admin_sms', 	__('SMS' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media sms', 'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'refund_admin_active',
				...
				Field::make('text',		'refund_admin_sms_recipient', __('Nomor SMS Penerima', 'sejoli'))
					->set_help_text(__('Gunakan tanda koma jika penerima ada lebih dari 1', 'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'refund_admin_active',
				...
				Field::make('textarea', 'refund_admin_sms_content', __('Konten', 'sejoli'))
					->set_help_text(__('Dengan mengosongkan isian ini, tidak akan ada notifikasi yang dikirimkan via sms', 'sejoli'))
				...
				Field::make('separator'	,'sep_refund_admin_whatsapp', 	__('WhatsApp' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media whatsapp', 'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'refund_admin_active',
				...
				Field::make('text',		'refund_admin_whatsapp_recipient', __('Nomor WhatsApp Penerima', 'sejoli'))
					->set_help_text(__('Gunakan tanda koma jika penerima ada lebih dari 1', 'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'refund_admin_active',
				...
				Field::make('textarea', 'refund_admin_whatsapp_content', __('Konten', 'sejoli'))
					->set_help_text(__('Dengan mengosongkan isian ini, tidak akan ada notifikasi yang dikirimkan via whatsapp', 'sejoli'))
    ...
    protected function set_content() {
		...
		$this->set_recipient_title  ('buyer', 'email', sejolisa_carbon_get_theme_option('refund_email_title'));
		$this->set_recipient_content('buyer', 'email', $this->set_notification_content(
															sejolisa_carbon_get_theme_option('refund_email_content'),
															'email',
                                                            'buyer'
														));

		if(!empty(sejolisa_carbon_get_theme_option('refund_whatsapp_content'))) :

            $this->set_enable_send      ('whatsapp', 'buyer', true);
			$this->set_recipient_content('buyer', 'whatsapp', $this->set_notification_content(
				                                                sejolisa_carbon_get_theme_option('refund_whatsapp_content'),
				                                                'whatsapp',
                                                                'buyer'
                                                              ));
        endif;

		if(!empty(sejolisa_carbon_get_theme_option('refund_sms_content'))) :
            $this->set_enable_send      ('sms', 'buyer', true);
			$this->set_recipient_content('buyer', 'sms', $this->set_notification_content(
                                            				sejolisa_carbon_get_theme_option('refund_sms_content'),
                                            				'sms',
                                                            'buyer'
                                            			));
        endif;
		...
		if(false !== sejolisa_carbon_get_theme_option('refund_admin_active')) :

            $this->set_enable_send('email', 'admin', true);
    		$this->set_recipient_title  ('admin', 'email', sejolisa_carbon_get_theme_option('refund_admin_email_title'));
    		$this->set_recipient_content('admin', 'email', $this->set_notification_content(
    															sejolisa_carbon_get_theme_option('refund_admin_email_content'),
    															'email',
                                                                'admin'
    														));

    		if(!empty(sejolisa_carbon_get_theme_option('refund_admin_whatsapp_content'))) :

				$this->set_enable_send        ('whatsapp', 'admin', true);
				$this->set_recipient_content  ('admin', 'whatsapp', $this->set_notification_content(
                                                    					sejolisa_carbon_get_theme_option('refund_admin_whatsapp_content'),
                                                    					'whatsapp',
                                                                        'admin'
                                                    				));
            endif;

    		if(!empty(sejolisa_carbon_get_theme_option('refund_admin_sms_content'))) :
                $this->set_enable_send      ('sms',     'admin', true);
				$this->set_recipient_content('admin',   'sms', $this->set_notification_content(
                            					sejolisa_carbon_get_theme_option('refund_admin_sms_content'),
                            					'sms',
                                                'admin'
                            				));
            endif;
    ...
    protected function check_recipients($media = 'email', $role = 'admin') {
        $recipients       = sejolisa_carbon_get_theme_option('refund_' . $role . '_' . $media . '_recipient');
