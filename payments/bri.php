...
final class BRI extends \SejoliSA\Payment{
     ...
     * @var string
     */
    protected $table = 'sejolisa_bri_transaction';
    ...
    public function __construct() {
        ...
        $this->id          = 'bri';
        $this->name        = __('BRI Auto Check Mutasi', 'sejoli');
        $this->title       = __('BRI Internet Banking', 'sejoli');
        $this->description = __('Pengecekan mutasi ke BRI Internet Banking. Invoice akan diaktifkan secara otomatis jika nilai pembayaran sesuai tepat dengan nilai order.', 'sejoli');
        $this->table       = $wpdb->prefix . $this->table;

        require_once(plugin_dir_path(__FILE__) . 'mutation/bri.php');

        $this->mutation = new \Mutasi\Bank\BRI;

        add_action('admin_init',                    [$this, 'register_transaction_table'],  1);
        add_action('admin_init',                    [$this, 'check_bri_mutation'],          1);
        add_action('wp_ajax_bri-check-connection',  [$this, 'check_available_account'],     1);
        add_action('sejoli/order/new',              [$this, 'save_unique_code'],            999);
        add_action('admin_footer',                  [$this, 'add_js_script'],               999);
        add_action('sejoli/bri/check-mutation',     [$this, 'check_mutation'],              1);
        add_filter('sejoli/payment/payment-options', [$this, 'add_payment_options'] );


		if(false === wp_next_scheduled('sejoli/bri/check-mutation')) :

			wp_schedule_event(time() + 60,'fourth_hourly','sejoli/bri/check-mutation');

		else :

			$recurring 	= wp_get_schedule('sejoli/bri/check-mutation');

			if('fourth_hourly' !== $recurring) :
				wp_reschedule_event(time() + 60, 'fourth_hourly', 'sejoli/bri/check-mutation');
			endif;
		endif;
     ...
     * Hooked via wp_ajax_bri-check-connection
     * @since   1.0.0
     * @return  json
     */
    ...
    /**
     * Display html content to check connection to BRI Internet Banking
     * @since   1.0.0
     * @access  protected
     * @return  string
     */
    protected function display_html_check() {
        ...
        ?>
        <div class="bri-check">
            <button type="button" name="button" class='bri-connection-check button'><?php _e('Cek Koneksi ke BRI Internet Banking', 'sejoli'); ?></button>
            <div class="bri-check-result sejoli-html-message" style='margin-top:18px;display:none;'>
    ...
    public function add_js_script() {
        ...
        (function( $ ) {
            'use strict';
            $(document).on('click', '.bri-connection-check', function(){
                $.ajax({
                    url : '<?php echo admin_url('admin-ajax.php'); ?>',
                    data : {
                        action : 'bri-check-connection',
                        username : $("input[name='carbon_fields_compact_input[_bri_briib_username]']").val(),
                        password : $("input[name='carbon_fields_compact_input[_bri_briib_password]']").val(),
                    },
                    dataType : 'json',
                    beforeSend : function() {
                        $('.sejoli-html-message')
                            ...
                            .addClass('info')
                            .show()
                            .html('<?php echo __('Mengecek koneksi dengan BRI Internet Banking', 'sejoli'); ?>');
    ...
    /**
     * Add BRI setup fields to general form
    ...
    public function get_setup_fields() {
        ...
        return [
            Field::make('separator', 'sep_bri_transaction_setting',	__('Pengaturan Cek Mutasi BRI', 'sejoli')),

            Field::make('html', 'sp_bri_transaction_info', 'Informasi')
                ->set_html(
            ...
            Field::make('checkbox', 'bri_transaction_active', __('Aktifkan metode transaksi ini', 'sejoli'))
                ->set_option_value('yes')
                ->set_default_value(false)
                ->set_help_text(__('Metode pembayaran ini membutuhkan akses username dan password ke BRI Internet Banking personal anda', 'sejoli')),

            Field::make('text',     'bri_transaction_unique_code', __('Maksimal kode unik', 'sejoli'))
                ...
                ->set_conditional_logic([
                    [
                        'field' => 'bri_transaction_active',
                        'value' => true
                    ]
                ]),

            Field::make('select',   'bri_transaction_operation', __('Pengoperasian kode unik', 'sejoli'))
                ->set_width(50)
                ->set_options([
            ...
            Field::make('text',     'bri_briib_username', __('Username BRI Internet Banking', 'sejoli'))
                ->set_required(true)
                ->set_conditional_logic([
                    [
                        'field' => 'bri_transaction_active',
            ...
            Field::make('text',     'bri_briib_password', __('Password BRI Internet Banking', 'sejoli'))
                ->set_required(true)
            ...
            Field::make('text',     'bri_briib_account', __('Nomor rekening BRI internet banking anda', 'sejoli'))
                ->set_conditional_logic([
                    [
                        'field' => 'bri_transaction_active',
                        'value' => true
            ...
            Field::make('html',     'bri_briib_check', __('Cek Koneksi ke BRI Internet Banking', 'sejoli'))
            ...
            Field::make('text',     'bri_account_owner', __('Nama pemilik rekening', 'sejoli'))
                ->set_required(true)
                ->set_conditional_logic([
                    [
                        'field' => 'bri_transaction_active',
            ...
            Field::make('text',     'bri_account_branch', __('Cabang BRI', 'sejoli'))
                ->set_required(true)
                ->set_conditional_logic([
                    [
                        'field' => 'bri_transaction_active',
            ...
            Field::make('select',   'bri_check_day',    __('Lama hari pengecekan mutasi', 'sejoli'))
    ...
    /**
     * Add payment options if BRI Internet banking active
    ...
    public function add_payment_options($options = array()) {
        ...
        $active = boolval( sejolisa_carbon_get_theme_option('bri_transaction_active') );

        if(true === $active) :
            $options['bri'] = [
                'label' => 'Bank BRI',
                'image' => SEJOLISA_URL . 'public/img/BRI.png'
            ];
        endif;
    ...
    protected function check_unique_code() {
        ...
        $operation = sejolisa_carbon_get_theme_option('bri_transaction_operation');
        ...
        if('' !== $operation) :
            ...
            $max_unique_code   = floatval(sejolisa_carbon_get_theme_option('bri_transaction_unique_code'));
            $this->unique_code = 1;
    ...
    public function save_unique_code(array $order_data) {
        ...
        if('bri' == $order_data['payment_gateway'] && !empty($this->unique_code)) :
    ...
    public function display_payment_instruction($invoice_data, $media = 'email') {
        ...
        $account_number = sejolisa_carbon_get_theme_option('bri_briib_account');
        $account_owner  = sejolisa_carbon_get_theme_option('bri_account_owner');
        $account_branch = sejolisa_carbon_get_theme_option('bri_account_branch');
        $content = '';
    ...
    public function display_simple_payment_instruction($invoice_data, $media = 'email') {
        ...
        $content .= sprintf(__("%s no rek %s", "sejoli"), 'BRI', $account_number );
    ...
    protected function render_mutation() {
        ...
        $response = $this->mutation->set_username(sejolisa_carbon_get_theme_option('bri_briib_username'))
                        ->set_password(sejolisa_carbon_get_theme_option('bri_briib_password'))
                        ->set_account(sejolisa_carbon_get_theme_option('bri_briib_account'))
                        ->set_type('kredit')
                        ->check_mutasi()
                        ->respond();
        ...
        if(false !== $response['valid']) :
            ...
            set_transient('sejoli-bri-last-check-account', $account_id);
    ...
    /**
     * Get orders that uses BRI payment gateway with status on-hold
     * @since   1.0.0
     * @return  void
     */
    protected function render_order() {
        ...
        $day         = intval(sejolisa_carbon_get_theme_option('bri_check_day'));
        ...
        $this->order_data = Capsule::table($this->table.' AS log')
                    ...
                    ->where('log.created_at', '>', date('Y-m-d', strtotime('-' . $bri . ' day')))
                    ->get();
    }

    /**
     * Check bank mutation
     * Hooked via action sejoli/bri/check-mutation
     * @since   1.0.0
     * @return  void
     */
     ...
     * @return  void
     */
    public function check_bri_mutation() {

        if(is_admin() && isset($_GET['sejoli-bank']) && 'bri' === $_GET['sejoli-bank']):
