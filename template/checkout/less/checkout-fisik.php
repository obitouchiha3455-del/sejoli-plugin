        ...
        <div class="grid-container grid-2">
           <div class="grid-item">
                ...
                <div class="tujuan-pengiriman">
                    <h2><?php _e('Tujuan Pengiriman', 'sejoli'); ?></h2>
                    <div class="ui form">
                        <div class="field">
            ...
            <div class="grid-item sticky-sidebar">
                ...
                <a id="detailPesanan" class="toggle-pesanan"><?php esc_html_e('LIHAT DETAIL PESANAN', 'sejoli'); ?> <i class="angle down icon"></i></a>
                
                <div id="detailPesananList" class="detail-pesanan-list">
                    ...
                    <div class="produk-dibeli">
                        ...
                        <table class="ui unstackable table">
                            <thead>
                                <tr>
                                    <th style="text-align:left"><?php _e('Produk', 'sejoli'); ?></th>
                                    <th style="text-align:right"><?php _e('Sub Total', 'sejoli'); ?></th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                ...
                <div class="floating-side">
                    ...
                    <?php if(false !== $product->form['detail_order']) : ?>
                    <div class="metode-pembayaran" style="margin:1em 0;">
                    <?php else: ?>
                    <div class="metode-pembayaran" style="margin:0;">
                    <?php endif; ?>
                        <a id="pilihMetode" class="toggle-metode"><?php esc_html_e('PILIH METODE PEMBAYARAN', 'sejoli'); ?> <i class="angle down icon"></i></a>
                    
                        <div id="metodeList" class="metode-list">
                            <h2 style="margin-top: 1em;"><?php _e('Pilih Metode Pembayaran', 'sejoli'); ?></h2>
                        ...
                        <div id="metodeTerpilih" class="metode-terpilih">
                            <span id="bankLogo"></span>
                            <a id="ubahMetode" class="ubah-metode"><?php esc_html_e('UBAH METODE PEMBAYARAN', 'sejoli'); ?> <i class="angle down icon"></i></a>
                        </div>
                    </div>
                    <div class="beli-sekarang eight wide column">
...
<script id="metode-pembayaran-template" type="text/x-jsrender">
    {{if payment_gateway}}
        {{props payment_gateway}}
            <div class="eight wide column">
                <div class="ui radio checkbox metode {{if key == 0}}checked{{/if}}">
                    ...
                    {{if prop.display_payment == true}}
                        <label><img src="{{>prop.image}}" alt="{{>prop.title}}"><span>{{>prop.title}}</span></label> 
                    {{else}}
        ...
        {{/props}}
    {{/if}}
</script><script id="metode-pengiriman-template" type="text/x-jsrender">
...
<script id="login-template" type="text/x-jsrender">
    ...
    {{else}}
        ...
        <div class="login-form-toggle">
            <?php if(false !== $product->form['detail_order']) : ?>
            <h2><?php _e('Data Pembeli', 'sejoli'); ?></h2>
            <?php else: ?>
            <h2><?php _e('Lengkapi Data', 'sejoli'); ?></h2>
            <?php endif; ?>
...
jQuery(document).ready(function($){
    ...
    document.getElementById('pilihMetode').addEventListener('click', function() {
        var metodeList = document.getElementById('metodeList');
        metodeList.style.display = metodeList.style.display === 'none' || metodeList.style.display === '' ? 'block' : 'none';

        var pilihMetode = document.getElementById('pilihMetode');
        pilihMetode.style.display = pilihMetode.style.display === 'none' || pilihMetode.style.display === '' ? 'none' : 'none';
    });

    document.getElementById('detailPesanan').addEventListener('click', function() {
        var detailPesananList = document.getElementById('detailPesananList');

        var icon = this.querySelector('i');
    
        if (icon.className.includes('angle down icon')) {
            icon.className = icon.className.replace('angle down icon', 'angle up icon');
        } else {
            icon.className = icon.className.replace('angle up icon', 'angle down icon');
        }

        detailPesananList.style.display = detailPesananList.style.display === 'none' || detailPesananList.style.display === '' ? 'block' : 'none';
    });

    $(document).on('change','input[type=radio][name=payment_gateway]',function() {
        var selectedImg = $(this).siblings('label').html();
        var selectedText = $(this).siblings('span').text();
        $('#bankLogo').html(selectedImg);
        $('#metodeTerpilih').show();
        $('#pilihMetode').hide();
        $('#metodeList').hide();
    });

    document.getElementById('ubahMetode').addEventListener('click', function() {
        document.getElementById('metodeTerpilih').style.display = 'none';
        document.getElementById('metodeList').style.display = 'block';
